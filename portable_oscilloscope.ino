...
float computeSampleRate() {
  unsigned long newest =
      timestamps[(bufIndex + NUM_SAMPLES - 1) % NUM_SAMPLES];
...
float estimateFrequencyFromZeroCrossings(int16_t minVal, int16_t maxVal) {
  ...
  float crossingTimes[maxCrossings];
  ...
  int prevIdx =
      (start + NUM_SAMPLES - 1) % NUM_SAMPLES;
  ...
  for (int i = 0; i < NUM_SAMPLES; ++i) {
    ...
    if ((v1 <= mid) && (v2 > mid)) {
      ...
      if (v2 != v1) {
        ...
        float t_cross_s =
            (t1 + frac * (float)(t2 - t1)) * 1e-6f;

        if (crossCount < maxCrossings)
          crossingTimes[crossCount++] = t_cross_s;
      }
    }
  ...
  for (int k = 1; k < crossCount; ++k) {
    ...
    double period =
        crossingTimes[k] - crossingTimes[k - 1];

    if (period > 0.0005 && period < 1.0) {
